...
# tema ggplot ----
tema_corrupcion <- theme(text = element_text(family = "IBM Plex Mono", color = color_texto, size = 5)) +
  theme(strip.text = element_text(size = opt_texto_axis),
        axis.title = element_text(size = opt_texto_plot, face = "italic"),
        axis.text = element_text(size = opt_texto_axis)) +
  theme(plot.background = element_rect(fill = color_fondo, color = color_fondo),
        panel.background = element_rect(fill = color_fondo3, color = color_fondo3),
...
resolucion = 130 #resolución de render de los gráficos
...
ui <- page_fluid(
  ...
  js_get_vertical_position(), # esto probocaba problemas en el ancho de los gráficos al escalar el tamaño de la ventana
...
server <- function(input, output, session) {
  ...
  # 
  # observe({
  #   message("posicion_scroll(): ", posicion_scroll())
  # })
  ...
  output$grafico_cep <- renderPlot({
    ...
    p +
      ...
      geom_point(size = opt_puntos_geom, alpha = .9) +
  ...
  output$grafico_cpi <- renderPlot({
    ...
    cpi |>
      ...
      geom_point(aes(y = cpi + 0.5, shape = cambio, fill = cambio, color = cambio),
                 size = opt_puntos_geom*0.66, alpha = 1) +
  ...
  output$torta_montos <- renderPlot({
    ...
    datos_montos |>
      ...
      # texto interior
      geom_text(aes(label = sector), position = position_stack(vjust = 0.5),
                angle = 90, hjust = 0.5, size = opt_texto_geom, fontface = "bold", color = "white") +
      # texto exterior
  ...
  output$mapa_centro_zoom <- renderPlot(mapa_rm_zoom())
  output$mapa_sur <- renderPlot(mapa_sur())
  output$mapa_rm <- renderPlot(mapa_corrupcion_rm())
  
  # este es el problema :(
  # output$mapa_interactivo_norte <- renderGirafe(mapa_norte())
  ...
  output$grafico_montos <- renderPlot({
    ...
    texto_eje_y = opt_texto_plot*1.7 #texto casos
    texto_montos = opt_texto_geom*1.7 #texto montos
